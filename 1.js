const LIST = [
  ...
  const makeArray = str => Array.from(str);
  const findFirst = list => list.find(i => NUMBERS.includes(i)) || 0;
  const findLast = list => findFirst(list.reverse());
  const toCodes = list => list.map(str => makeArray(str)).map(arr => ''.concat(findFirst(arr), findLast(arr)))
  const CODES = toCodes(LIST)
  const NUMBER_CODES = CODES.map(c => parseInt(c))
  const SUM = NUMBER_CODES.reduce((acc, curr) => acc + curr, 0)
  console.log(SUM)